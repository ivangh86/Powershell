...
if ($ExisteFichero -eq $True){del $ServerPerfect
    New-Item $ServerPerfect -ItemType file}

else { New-Item $ServerPerfect -ItemType file}
...
ForEach($computername in $servers){
 ...
 if ($serverlist) {  $Computer =  $computername }
    ...
    if ($result -ne "SUCCESS") { $tests.Ping = "NOK"  
            ...
            write-host "Maquina Operativa:$result"
         ...
         $tests.Ping = "OK" ;write-host "Maquina Operativa:$result"
...
#############################################################################   
#Revisamos si a nivel de WMI se pueden realizar consultas y qu
$Serverinvoke = @()
$Serverwmi = @()
 ForEach($Computer in $servers){

 
 if ($serverlist) {  $Computer =  $Computer}

 else { $Computer = $Computer.MachineName.Substring(6)}
     ...
     try {
        ...
        if ($?) {
            ...
            Write-Output "${computer}: WMI funciona correctamente"  <#$MemSizeGB"#> 
    ...
    $objVM = New-Object -TypeName psobject
    $objVM | Add-Member -Name "Estado" -Value "REDProblemaWMI" -MemberType NoteProperty
    $objVM | Add-Member -Name "Server" -Value $Computer -MemberType NoteProperty
    ...
    $Serverwmi += $objVM
            ...
            Add-Content -value $Computer -Path $ServerPerfect
            write-host "Funciona el Invoke-Command"
            }
  
            else {
             write-host "problema Invoke-Command $Computer" 
            $objVM1 = New-Object -TypeName psobject
            $objVM1 | Add-Member -Name "Estado" -Value "REDErrorInvoke" -MemberType NoteProperty
            $objVM1 | Add-Member -Name "Server" -Value $Computer -MemberType NoteProperty
...
#Cogemos los Servidores que realmente estan bien (responden a Ping y Funciona el WMI)
   ...
   ForEach($computername in $ServerPerfect){
        ...
        $wmiObjects | Foreach {
          ...
          #Porcentage Procesos           
            ...
            #############################################################################
            #Memoria
            #Si no queremos medias usaremos esta sintaxis para la Memoria
            #JOB 5
        ...
        <#
            ...
            #>
              ...
              ##################################################
              #Si queremos ver la cola de disco sin medias
              #JOB 9
              $diskcounterq =""
	          $diskcounterq = Get-Counter -Counter "\LogicalDisk(C:)\Avg. disk queue length" | select CounterSamples
      ...
      $TimeOut = 3000 
        ...
        Get-Job | Wait-Job -Timeout $TimeOut | select Name,State,Location | ft -AutoSize
...
$resultado = @()
        ...
        for ($i=0;$i -lt ($job).Count;$i++){    
            ...
            write-host "DiskQueue:"$jobresult[9]
            write-host "Uptime:"$jobresult[10]
        
            $objVM = New-Object -TypeName psobject
            $objVM | Add-Member -Name "Ping" -Value "GREENOK" -MemberType NoteProperty
            $objVM | Add-Member -Name "Server" -Value $Computer -MemberType NoteProperty
            $objVM | Add-Member -Name "CPU Average / 30 sec" -Value $jobresult[2] -MemberType NoteProperty
            ...
            $objVM | Add-Member -Name "Memory Usage" -Value $jobresult[5] -MemberType NoteProperty
            ...
            $objVM | Add-Member -Name "%Ocupado C:" -Value $jobresult[6].Substring(0) -MemberType NoteProperty
            $objVM | Add-Member -Name "%Ocupado D:" -Value $jobresult[7].Substring(0) -MemberType NoteProperty
            $objVM | Add-Member -Name "%Ocupado E:" -Value $jobresult[8].Substring(0) -MemberType NoteProperty
            $objVM | Add-Member -Name "DiskQueue" -Value $jobresult[9] -MemberType NoteProperty
            ...
            $objVM | Add-Member -Name "UpTime" -Value $jobresult[10] -MemberType NoteProperty
...
$Elapsed = ""
$formateelapsed = ""
$Elapsed=(Get-Date)-$firstdate
$formateelapsed = echo $Elapsed.Days "Dias ", $Elapsed.Hours "Horas ", $Elapsed.Minutes  "Minutos ",  $Elapsed.Seconds "Segundos"

#$formateelapsed > $timescript
$hosted = hostname
$resultado = $resultado | ConvertTo-Html -Head $headformat -Body "<H3>Server Health Report System Status XenApp 7.15 BS - $dia $hora</H3> <H4>Tiempo Trasncurrido Script: $formateelapsed</H4> <H5>Ejecutado en: '$hosted';  Ubicado en: $currentDir; Nombre: CPU_Average_XenDesktop.ps1; Parametros: -sendmail -serversilo 'SDI W2K8R3 SSCC IE10 PRO' </H5>"  |  foreach {
...
$smtpCC = "gomez-hernandez@dxc.com"
