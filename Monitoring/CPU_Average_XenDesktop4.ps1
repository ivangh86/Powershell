...
#############################################################################   
#Revisamos si a nivel de WMI se pueden realizar consultas
...
$ServerPerfect = Get-Content $ServerPerfect
   ...
   ForEach($computername in $ServerPerfect){
      ...
      $TimeOut = 1000 
        ...
        Get-Job | Wait-Job -Timeout $TimeOut | select Name,State,Location
