...
Process 
{
    ...
    function help
    {
		...
		Write-Host "Using the `"enable`" Action with -ListenerType enables a specified listener type (http or https).`n"
		Write-Host "Using the `"enable`" Action with a username sets the user's rights as they pertain to WinRM i.e. Remote WMI Access and Write-Host has rights to the WMI plugin which is necessary for SID lookup by the colector`n"
...
#		$setparams.GetEnumerator() | Sort-Object Value -descending
				...
				{	
					If ($extension.Oid.FriendlyName -eq "Enhanced Key Usage")
					{
						...
						foreach ($enhancedKeyUsage in $enhancedKeyUsageExtension.EnhancedKeyUsages)
						{
							...
							{
								if($cert.Subject -match $fqdn -or $cert.Subject -eq $env:computername)
								{
   ...
   { 	param
	...
	log "winrmconfig script version 1.17"
	...
	if($global:reportMode -eq $false -and $lType -ne "")
	{
		...
		if($lType -eq "https")
		{
			...
			if($firewallStatus -match "Running" -and $global:httpsPort -eq $WINRM_DEFAULT_HTTPS_PORT) #we can quickconfig even if there is an existing port
			{
		...
		elseif($lType -eq "http")
		{
			...
			else
			{
				...
				{																								# and it's not same port as the one we are setting
					...
					deleteHTTPSListener																			#port then we must assume its meant to be overwritten so delete it
				}
