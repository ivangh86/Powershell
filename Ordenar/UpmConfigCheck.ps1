#
# Copyright Citrix Systems 2011-2017
#
...
Supported Platforms: Windows XP, Windows Server 2003, Windows Vista, Windows 7, Windows Server 2008 R2, Windows 8/8.1, Windows Server 2012 R2.  Support for this script on a platform does not imply that Citrix supports any specific features, products or combination of products on that platform, including but not limited to UPM (Citrix Profile Management), XenApp, XenDesktop, Xen Server, Personal vDisk and/or VDI-in-a-Box.
...
$upmCheckVersion = 'UPM Configuration checking tool version $Revision: #145 $'
$scriptRunDate = date
...
$realPathForDefaultExclusionReg = @{      
    ...
    "ExclusionDefaultRegistry03" = "Software\Microsoft\Speech_OneCore"
}
...
# Query UPM Driver Version in windows
...
if(Test-path $UPMDriver){
	...
	if($upmjitversion -lt $UPMBaseDriverVersion ) {
		"*** Warning: An older version of UPM driver is being used. UPM may not function properly " | CaptureRecommendations -PolicyName "" -Reason "UPM driver in use is an older version than expected."
 	}
}
else{
	"*** ERROR: UPM driver is not installed " | CaptureRecommendations -PolicyName "" -Reason "UPM driver is not installed. UPM may not work properly"
}
...
ReportEnvironment "UPM Driver" "DisplayVersion" $upmjitversion
...
if( $pol){
	switch ( $pol ) {
		...
		default { "*** MigrateWindowsProfilesToUserStore is unset and will default to None" }
	}
}
else{
	"*** MigrateWindowsProfilesToUserStore is unset and will default to None"
}
...
"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
"/ Checking LogonExclusionCheck /"
"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
$LogonExclusionCheck = GetEffectivePolicyFlag -policyName "LogonExclusionCheck" -defaultSetting 0 -AsNumber
switch ($localProfileConflicts.Value) {
  0 { "LogonExclusionCheck is set to Sync excluded files or folders" }
  1 { "LogonExclusionCheck is set to Ignore excluded files or folders" }
  2 { "LogonExclusionCheck is set to Delete excluded files or folders" }
}
...
function CheckPackageAndUrclassDat
{
    $bUrclassDatExcluded = $false;
    ...
    if($bUrclassDatExcluded -ne $bPackagesExcluded)
    {
     "*** warning - Start menu might not work on Win10 or Win2016"  | CaptureRecommendations -PolicyName "" -Reason "The !ctx_localappdata!\Microsoft\Windows\UsrClass.dat* file and the !ctx_localappdata!\Packages folder must be included and excluded at the same time on Windows 10 or on Windows 2016."
    }
}
#
#For Win10/Win2016 platform, urclass.dat and packages should be included or excluded in the same time
function CheckSpeechOneCore
{
    ...
    {
     "*** warning - File type associations (FTA) might not work on Windows 10 or on Windows 2016"  | CaptureRecommendations -PolicyName "" -Reason "The Software\\Microsoft\\Speech_OneCore file must not be excluded on Windows 10 or on Windows 2016."
    }
}
#
...
elseif(($winver -eq "Win10") -or ($winver -eq "Win2016"))
{
    CheckPackageAndUrclassDat
    CheckSpeechOneCore
}
...
"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
"/ Checking LastKnownGoodRegistry        /"
"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
$polName = "LastKnownGoodRegistry"
PreferPolicyFlag -policyName $polName -defaultSetting 1 -preferredSetting 1 -Reason "By default, the NTUSER.DAT backup policy is enabled to back up the last good load NTUSER.DAT and roll back to it if NTUSER.DAT is corrupt." 
$LastKnownGoodRegistry = Get-PolicyRecordProperty -PolicyName $polName -PropertyName EffectiveValue


"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
"/ Checking CEIPEnabled                  /"
"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
$polName = "CEIPEnabled"
PreferPolicyFlag -policyName $polName -defaultSetting 1 -preferredSetting 1 -Reason "By default, the Customer Experience Improvement Program is enabled to help improve the quality and performance of Citrix products by sending anonymous statistics and usage information." 
$CEIPEnabled = Get-PolicyRecordProperty -PolicyName $polName -PropertyName EffectiveValue

"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
"/ Checking PSMidSessionWriteBackReg     /"
"/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/"
...
ValidateList -list $polExclusionReg -policyName $polName
...
ValidateList -list $polExclusionDir -policyName $polName
#
...
ValidateList -list $polDefaultExclusionReg -policyName $polName
...
ValidateList -list $polDefaultExclusionDir -policyName $polName

#
# Let's check a policy - StreamingExclusionList
#
...
#
"--------------------------------------------------------"
"Compare Streaming exclusion list with Citrix recommendation"
"--------------------------------------------------------"
...
if ($prov -ne $physical) {
  ...
  $reason = "When running Profile Management in a provisioned environment, Citrix recommends that you save log files in a persistent location provided by Citrix Provisioning Services (PVS) or by the Citrix Machine Creation Service (MCS), or to a network location. This will prevent logs from being lost in the event of a serious failure. The default maxLogSize is 10M. You must restrict the log file size to a maximum of 7 MB, which is half the PVS/MCS persistent location size."
}
if ($null -ne $vdiInfo) {
...
if ($polcount -gt 0) {
  ...
  if ($polmatch -ne $cookieFolderCount) {
    "*** Cookie folder ( $printForCookieFolder ) should be specified exactly once.  It has been specified $polmatch times." | CaptureRecommendations -PolicyName $polName -Reason "This causes policy `"process internet cookie file on logoff`" not work and win10 start menu crash ."
  } else {
    "Cookie folder ( $printForCookieFolder )is included once in MirrorFoldersList"
  }
}
"-----------------------------------------"
