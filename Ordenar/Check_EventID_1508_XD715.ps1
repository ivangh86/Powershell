...
Foreach ($Server in $Serverlist) 
{
    ...
    $events = Get-WinEvent -Computername $Machine -FilterHashtable @{Logname=$Source ;ID=@($ID1,$ID2); providername= $ProviderName ; StartTime=$days} -EA 0
    ...
    if($events){
        ForEach($event in $events){
            ...
            "$machine  ; $($Server.DesktopGroupName) ; $($event.ID) ; $($event.TimeCreated); $($event.Message)"  | Out-file -append $PathAttach
