...
Foreach ($Server in $Serverlist) 
{
    ...
    $events = Get-WinEvent -Computername $Machine -FilterHashtable @{Logname=$Source ;ID=@($ID1); StartTime=$days} -EA 0
    
    if($events){
        ForEach($event in $events){

            If ($($event.Message) -like "*Y181472*"){
