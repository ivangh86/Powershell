		...
		If($GotNics)
		{
			ForEach($nic in $nics)
			{
				...
				If($? -and $Null -ne $ThisNic)
				{
					...
					If($xIPAddress.Count -gt 1)
					{
						$obj | Add-Member -MemberType NoteProperty -Name DCIpAddress -Value $xIPAddress[0]
					}
					Else
					{
						$obj | Add-Member -MemberType NoteProperty -Name DCIpAddress -Value $xIPAddress
					}
...
If($DCDNSInfo)
{
	...
	ForEach($Item in $DCDNSIPInfo)
	{
		If($MSWord -or $PDF)
		{
			...
			DCName = $Item.DCName;
			DCSite = $Item.DCSite;
			DCIpAddress = $Item.DCIpAddress;
			DCDNS1 = $Item.DCDNS1
			}
			If($Null -ne $Item.DCDNS2)
            {
			    $WordTableRowHash += @{ 
			    DCDNS2 = $Item.DCDNS2
			    }
            }
			If($Null -ne $Item.DCDNS3)
            {
			    $WordTableRowHash += @{ 
			    DCDNS3 = $Item.DCDNS3
			    }
            }
			If($Null -ne $Item.DCDNS4)
            {
			    $WordTableRowHash += @{ 
			    DCDNS4 = $Item.DCDNS4
			    }
		...
		ElseIf($Text)
		{
			Line 1 "DC Name`t: " $Item.DCName
			Line 1 "Site Name: " $Item.DCSite
			Line 1 "IP Address`t: " $Item.DCIpAddress
			Line 1 "Preferred DNS`t: " $Item.DCDNS1
			Line 1 "Alternate DNS`t: " $Item.DCDNS2
			Line 1 "DNS 3`t: " $Item.DCDNS3
			Line 1 "DNS 4`t: " $Item.DCDNS4
			Line 0 ""
		}
		ElseIf($HTML)
		{
	...
	If($MSWord -or $PDF)
	{
		...
		-Columns DCName, DCSite, DCIpAddress, DCDNS1, DCDNS2, DCDNS3, DCDNS4 `
		-Headers "DC Name", "Site", "IP Address", "Preferred DNS", "Alternate DNS", "DNS 3", "DNS 4" `
		-AutoFit $wdAutoFitContent;
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUNHBd9LrJ3WZZ7azzUoziQ62Y
# D0igghtxMIIDtzCCAp+gAwIBAgIQDOfg5RfYRv6P5WD8G/AwOTANBgkqhkiG9w0B
...
# 9w0BCQMxDAYKKwYBBAGCNwIBBDAjBgkqhkiG9w0BCQQxFgQU+UFKgICpkpzdSm62
# z1hSl87RVL0wDQYJKoZIhvcNAQEBBQAEggEAeyUlwqAjCXjMsGiVQaan5FoYxwzQ
# D0bcgRIGRCXEk84SpdE8X4JTUweD1OPwY7vK5KyhaF1jnTDlIStLZWAcz83r1ozW
# JEj5PC1bLSv+nzx6/x1bOJjuE8VWnT64AWds/Myg4CAV5SsDdjtpUHFfxLn7EzDq
# ZVDG4d/fxXEnEe0BFb3OCPKLTy9mFlAaIlt4kfhTacDM3bXwj48Dpr38lVjJhZzN
# ybez+E0oLC5qvKARkiN0ImqWw8vDlNRkYMimntbdEE8n+LzX+VV6nA7FDzqC9dnB
# kT9svGIhOZEoyeXeAcLVJO3ptAjWOB4BNoPaaO+IU5GEo5H/XEe51mZuXqGCAg8w
...
# 9w0BCQUxDxcNMTYwNDIxMTIzNDQ5WjAjBgkqhkiG9w0BCQQxFgQUO6JLV8BX50Nr
# fDDRcjpdZij+mJ8wDQYJKoZIhvcNAQEBBQAEggEAK5HozYW8kbrvs6iT8hAjrj32
# rHBj3FK4T9Ig/BUbWCSUHgVVYDqwpfIx27DEltrbg5wOHyvn83O6cW4HL4tkCFjC
# TexpD7fohyd5Win6JRWAsKwCwg3JhqSAa9cAn9Lv1AT19UQ3vDJ3BWkC4tC5t6iE
# wlx24kN/Ie7AkPHLvpwpn2ausOQRYDw0CBTbiFiGC05lRcMCCba8vP91sQzWJIP+
# 2+JXm3zfMznXsoE3ZZvLgfGEwQ9xuC9l6WFftDYrFgv52qXisDwinEYUDt/AxLZb
# Ayp5LCcE1ZDhJA8VDKyqyFZ9t3kcEhjKOL5GIcWMs7z2SHBOwX8A+1U+eQp1gg==
