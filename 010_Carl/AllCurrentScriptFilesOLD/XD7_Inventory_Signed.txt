...
.PARAMETER CompanyName
	...
	The default value is contained in HKCU:\Software\Microsoft\Office\Common\UserInfo\CompanyName or
...
.EXAMPLE
	PS C:\PSScript > .\XD7_Inventory.ps1 -Summary
	
	Creates a Summary report with no detail.
	Will use all Default values.
	
	If MSWORD or PDF is specified then
		HKEY_CURRENT_USER\Software\Microsoft\Office\Common\UserInfo\CompanyName="Carl Webster" or
		HKEY_CURRENT_USER\Software\Microsoft\Office\Common\UserInfo\Company="Carl Webster"
		$env:username = Administrator
...
Function OutputDeliveryGroupTable 
{
	...
	ForEach($Group in $AllDeliveryGroups)
	{
		...
		{
			"DesktopsOnly"		{$Global:TotalDesktopGroups++; Break}
			"AppsOnly"			{$Global:TotalApplicationGroups++; Break}
			"DesktopsAndApps"	{$Global:TotalAppsAndDesktopGroups++; Break}
			Default				{Break}
		}
			...
			While(Test-Path $TempFile)
			{
				...
				If($cnt -gt 1)
				{
					...
					If($cnt -gt 2)
					{
						...
						#Find out if excel is running in our session
						$excelprocess = ((Get-Process 'Excel' -ea 0)|?{$_.SessionId -eq $SessionID}).Id
	...
	If($Null -ne $HDXPolicies)
	{
		...
		ForEach($Policy in $HDXPolicies)
		{
			...
			ElseIf($Text)
			{
				...
				Line 2 "Enabled`t`t: " $Policy.Enabled.ToString()
				...
				If(![String]::IsNullOrEmpty($filters))
				{
					...
					ForEach($Filter in $Filters)
					{
						...
						ElseIf($Text)
						{
							...
							Line 2 "Type`t: " $tmp
							Line 2 "Enabled`t: " $filter.Enabled.ToString()
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU5paWuzp3Xkq94np2dB2K1s1y
# JnagghtxMIIDtzCCAp+gAwIBAgIQDOfg5RfYRv6P5WD8G/AwOTANBgkqhkiG9w0B
...
# 9w0BCQMxDAYKKwYBBAGCNwIBBDAjBgkqhkiG9w0BCQQxFgQU+dXQBf7f5EibgcHd
# xGEu04UtSLwwDQYJKoZIhvcNAQEBBQAEggEAP+2IWyWT9I6urrKua8LS7mUIBl5c
# b7Yau68GdM0xBJWz/2fa+0FJ/xIJkzvcE5j/x7+q6eUJhtypwMBgwf05r/kgqJMc
# ckLMstWAKHcvMughh4ds8gb1vHQHETddvOvulcK0c/gsLeyWn0zryKp4rrpQ2n/f
# czQeg2dnRC9Yp9tQYbF+kZOMmR6fHYtJNmvD6A+VgzSrfjgpMR0tVIiI3QWkc1kQ
# lVFs+2E09s5/qhbFr8moj/8xRTMe83YmEHX8htzQ3KeEcM0iYVjCpDMeUoKbvrOK
# k064YOD+HIB9L1JalJVcKUNwn8HxLOefPy0ZZzk/CcCusS+GCOkGtBTWiaGCAg8w
...
# 9w0BCQUxDxcNMTcwMjEzMTQ1NjI1WjAjBgkqhkiG9w0BCQQxFgQUm83TPitduBRt
# bdxf1jR/cp8uAMUwDQYJKoZIhvcNAQEBBQAEggEAUnlDurvUCwTjL9zF1L2BnyKs
# +5nTJ5RZi+W3jWxcEVUy6s+I4Ys9TZ2/viWHu1vdhF89l/+4xTHQa6JqVSv+zMaG
# WWSf5KZ8Hvwl5H0h+stZ7tPlWw9fAfBYaDu3lu7ZHEtPZMslTi6DRpsAsEoJgXLk
# ACgZ4OPQcxQPQUILAmiXBXwWWOnshWn/GdSp9tgy1uyS8dAOFWG1pLycW/wlqOyz
# +PDf9N4IvHyOrK+CTXcHedgZjqnS6cDbWE1tjUBkLy6vVpHUHs1WONS1jMLhkYna
# 1wDYp8OQPTb8vo23G5Rl9ztHGpNKizfl83LSdX7A3H0h0cIfOtF94Mi0PWCicg==
