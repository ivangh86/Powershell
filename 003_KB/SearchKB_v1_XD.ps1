...
#      24/05/2017 - Se añade que revise si tiene los PSSnapin de Citrix y en caso que no esté, que los instale.
#
#####################################################
...
$Servidores = Get-BrokerDesktop -MaxRecordCount | Where-Object {$_.DNSName -like "*vdnbpv0001p*" } | select-object DNSName
...
TRY {
    ...
    $Serv.Eleccion = $Servidor.ElectionPreference
        ...
        if ($MaquinaEncendida -eq "True") {
            ...
            $Hotfixes = get-wmiobject -class win32_quickfixengineering -computer $Servidor.Nombre | select-object HotFixID | Select-String 'KB4012212','KB4012213','KB4012214','KB4012598'
            $Serv.Hotfixes  = ""
                Write-Host $Hotfixes
                if (!$Hotfixes)
