...
param ($Servidor)
       ...
       # filtramos si la máquina esta apagada para descartarla de la busqued 
        ...
        #Maquina Sin HotFix
                ...
                Write-Host "$Servidor","FALTA KB" >> "$scriptDir\test.csv"
                }
                    ...
                    Write-Host "$Servidor","$Hotfixes" >> "$scriptDir\test.csv"
                    }
                }
        }
        ...
        Write-Host "$Servidor","APAGADA" >> "$scriptDir\test.csv"
        }   
...
Get-Job | Wait-Job | Receive-Job | Set-Content

####Procesamos el archivo de salida
$ServidoresContenido = "$scriptDir\test.csv"
